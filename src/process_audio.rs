...
pub fn process() -> io::Result<Vec<Vec<f32>>> {
    ...
    let processed: Vec<f32> = process_audio(samples, sample_rate);
    
    println!("Success LPF! Samples: {}", processed.len());

    // 2. Windowing and FFT
    let window_size = 1024;
    let hop_size = 512; // 50% overlap for better resolution
    let mut spectrogram = Vec::new();

    for chunk in processed.windows(window_size).step_by(hop_size) {
        let mut windowed = chunk.to_vec();
        apply_hamming(&mut windowed);
        
        let magnitudes = raw_fft(windowed);
...
fn apply_hamming(frame: &mut Vec<f32>) -> Vec<f32> {
